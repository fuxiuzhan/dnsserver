...
@Service
public class MX implements IRecordProcess {
	...
	public void processReq(Message message, IoSession session) throws IOException {
		...
		if (mIps != null) {
			...
			for (String host_s : mip) {
				...
				Record answer = new MXRecord(record.getName(), record.getDClass(), Params.getDnsConfig().getTtl(), 1, name);
				outdata.addRecord(answer, Section.ANSWER);
			}
			byte[] buf = outdata.toWire();
			IoBuffer response = IoBuffer.wrap(buf);
			session.write(response);
			logger.info("MMXRecord->" + host);
			Params.getMqService().sendDnsLog(host, "MX", session);
			return;
		}
		// 查询A记录是否存在缓存中
		String ipString = Params.getRedis().getKey("MX" + host);
		if (ipString == null) {
			// 查询A记录缓存未命中，开始向目标Server更新
			Record[] records = new DNSLookup().Lookup(host, Type.MX);
			if (records != null) {
				// 想目标Server更新成功
				...
				IoBuffer response = IoBuffer.wrap(buf);
				session.write(response);
				Params.getRedis().setKey("MX" + host, sBuilder.toString());
				// 向队列发送更新信息，更新数据库
				Params.getMqService().sendMessageUpdate("MX|" + host + "|" + sBuilder.toString());
			} else {
				// 向目标Server更新失败，开始查询DB
				logger.error("MXRecord Error");
				// read database
				ipString = WrapDnsService.querryDNS(host, "MX");
				...
				if (ipString != null) {
					...
					IoBuffer response = IoBuffer.wrap(buf);
					session.write(response);
					Params.getRedis().setKey("MX" + host,ipString, Params.getDnsConfig().getTempLiveTime());
				} else {
					// 查询DB失败，返回默认值
					...
					IoBuffer response = IoBuffer.wrap(buf);
					session.write(response);
					Params.getRedis().setKey("MX" + host, Params.getDnsConfig().getServerName(), Params.getDnsConfig().getTempLiveTime());
				}
			}
		} else {
			// 命中缓存
