...
@Service
public class AAAA implements IRecordProcess {
		...
		if (mIps != null) {
			...
			for (String ip : mip) {
				if (IPAddressUtil.isIPv6LiteralAddress(ip)) {
					...
					Record answer = new AAAARecord(record.getName(), record.getDClass(), Params.getDnsConfig().getTtl(), answerip);
			...
			Params.getMqService().sendDnsLog(host, "AAAA", session);
			logger.info("MAAAARecord->" + host);
			return;
		}
		// 查询A记录是否存在缓存中
		String ipString = Params.getRedis().getKey("AAAA" + host);
		if (ipString == null) {
			// 查询A记录缓存未命中，开始向目标Server更新
			Record[] records = new DNSLookup().Lookup(host, Type.AAAA);
			if (records != null) {
				// 想目标Server更新成功
				...
				IoBuffer response = IoBuffer.wrap(buf);
				session.write(response);
				Params.getRedis().setKey("AAAA" + host, sBuilder.toString());
				Params.getMqService().sendMessageUpdate("AAAA|" + host + "|" + sBuilder.toString());
				// 向队列发送更新信息，更新数据库
			} else {
				// 向目标Server更新失败，开始查询DB
				logger.error("AAAARecord Error");
				// read database
				ipString = WrapDnsService.querryDNS(host, "AAAA");
				...
				if (ipString != null) {
					...
					IoBuffer response = IoBuffer.wrap(buf);
					session.write(response);
					Params.getRedis().setKey("AAAA" + host,ipString, Params.getDnsConfig().getTempLiveTime());
				} else {
					// 查询DB失败，返回默认值
					InetAddress answerip = InetAddress.getByName("0::" + Params.getDnsConfig().getDefaultTarget());
					Record answer = new AAAARecord(record.getName(), record.getDClass(), Params.getDnsConfig().getTtl(), answerip);
					outdata.addRecord(answer, Section.ANSWER);
					byte[] buf = outdata.toWire();
					IoBuffer response = IoBuffer.wrap(buf);
					session.write(response);
					Params.getRedis().setKey("AAAA" + host, "0::" + Params.getDnsConfig().getDefaultTarget(), Params.getDnsConfig().getTempLiveTime());
				}
			}
		} else {
			// 命中缓存
			String[] ips = ipString.split(",");
			for (String ip : ips) {
				if (ip != null && IPAddressUtil.isIPv6LiteralAddress(ip)) {
					InetAddress answerip = InetAddress.getByName(ip);
					Record answer = new AAAARecord(record.getName(), record.getDClass(), Params.getDnsConfig().getTtl(), answerip);
					outdata.addRecord(answer, Section.ANSWER);
				}
